...
export const foodDelivery = (latitude, longitude) => fetch(
...
export const foodActivity = (latitude, longitude) => fetch(
...
export const getRatingList = (shopid, offset, tag_name = '') => fetch('/ugc/v2/restaurants/' +
  shopid + '/ratings', {
    has_content: true,
    offset,
    limit: 10,
...
export const sendMobile = (sendData, captcha_code, type, password) => fetch(
...
export const postAddAddress = (userId, address, address_detail, geohash, name, phone, phone_bk,
  poi_type, sex, tag, tag_type) => fetch('/v1/users/' + userId + '/addresses', {
  address,
  address_detail,
  geohash,
  name,
...
export const vipCart = (id, number, password) => fetch('/member/v1/users/' + id +
  '/delivery_card/physical_card/bind', {
    number,
    password
  }, 'POST')
...
export const getHongbaoNum = id => fetch('/promotion/v2/users/' + id +
...
export const getExpired = id => fetch('/promotion/v2/users/' + id +
...
export const exChangeHongbao = (id, exchange_code, captcha_code) => fetch('/v1/users/' + id +
  '/hongbao/exchange', {
    exchange_code,
    captcha_code,
  }, 'POST');
...
export const getOrderDetail = (user_id, orderid) => fetch('/bos/v1/users/' + user_id + '/orders/' +
  orderid + '/snapshot');
...
export const deleteAddress = (userid, addressid) => fetch('/v1/users/' + userid + '/addresses/' +
  addressid, {}, 'DELETE')
